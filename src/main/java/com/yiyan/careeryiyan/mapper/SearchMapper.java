...
public interface SearchMapper {
    ...
    @Select("select *,r.ep_id as ep_id ,\n" +
            "       MATCH(r.rc_name) AGAINST(#{recruitmentName} IN NATURAL LANGUAGE MODE) as score\n" +
            "from recruitment r join enterprise e on r.ep_id = e.ep_id\n" +
            "         where e.is_delete=0 and r.is_delete=0 and MATCH(r.rc_name) AGAINST(#{recruitmentName} IN NATURAL LANGUAGE MODE)\n" +
            "order by score desc\n" +
    ...
    //单表查询，不需要手动映射名字
    @Select("select * ,MATCH(ep_name) AGAINST(#{enterpriseName} IN NATURAL LANGUAGE MODE) as score\n" +
            "from enterprise\n" +
            "where MATCH(ep_name) AGAINST(#{enterpriseName} IN NATURAL LANGUAGE MODE) and is_delete=0\n" +
            "order by score desc\n" +
            "limit #{pageSize},#{pageNum};")
    ...
    @Select("select *,u.user_id as user_id,e.ep_id=eu.ep_id as ep_id," +
            "MATCH(user_name) AGAINST(#{userName} IN NATURAL LANGUAGE MODE) as score\n" +
            "from user u join enterprise_user eu on u.user_id = eu.user_id join enterprise e on eu.ep_id = e.ep_id\n" +
            "where MATCH(user_name) AGAINST(#{userName} IN NATURAL LANGUAGE MODE) " +
    ...
    @Select("select count(*)\n" +
            ...
            "and MATCH(user_name) AGAINST(#{userName} IN NATURAL LANGUAGE MODE);")
